...
router.get('/:id', auth.authenticate, async(req, res, next) => {
  try {
      ...
      res.status(500).send("We encountered a problem while fetching the user you are trying to find ");
      console.log(error);
  }
});
...
router.post('/', validateUser, async(req, res, next) => {
  try {
    ...
    res.status(200).send( { name: user.first_name + " " + user.last_name, email: user.email } );
...
router.put('/:id', [auth.authenticate, validateUser], async(req, res, next) => {
  try {
    const validUser = validationResult(req)
    if(!validUser.isEmpty()){
      return res.send( { errors: validUser.array() } );
    ...
    if (req.params.id)
    {
      ...
      await Users.updateOne(
        ...
        res.status(200).send( { success:true, message:`Record ${req.params.id} updated successfully`,  } );
    }
    else{
...
router.delete('/:id', auth.authenticate, async(req, res, next) => {
  try {
    if (req.params.id)
    {
      ...
      res.status(202).send({success: true, message: "User deleted successfully", email:del.email});
    }
    else
    {
      res.status(500).send("Record id was not provided");
    ...
    res.status(500).send("There was an error deleting the user. User not found");
    console.log(error);    
  }
